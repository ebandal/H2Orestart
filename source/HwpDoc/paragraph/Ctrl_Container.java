...
public class Ctrl_Container extends Ctrl_GeneralShape {
    ...
    public Ctrl_Container(HwpxFile hwpx, String ctrlId, Node node, int version) throws NotImplementedException {
        ...
        for (int i=0; i<nodeList.getLength(); i++) {
            ...
            case "hp:container":    // 컨테이너 객체
                ctrl = new Ctrl_Container(hwpx, "noc$", child, version);
                list.add(ctrl);
                break;
            case "hp:line":     // 그리기 객체 - 선
                ctrl = new Ctrl_ShapeLine(hwpx, "nil$", child, version);
                list.add(ctrl);
                break;
            case "hp:rect":
                ctrl = new Ctrl_ShapeRect(hwpx, "cer$", child, version);
                list.add(ctrl);
                break;
            case "hp:ellipse":
                ctrl = new Ctrl_ShapeEllipse(hwpx, "lle$", child, version);
                list.add(ctrl);
                break;
            case "hp:arc":
                ctrl = new Ctrl_ShapeArc(hwpx, "cra$", child, version);
                list.add(ctrl);
                break;
            case "hp:polygon":
                ctrl = new Ctrl_ShapePolygon(hwpx, "lop$", child, version);
                list.add(ctrl);
                break;
            case "hp:curve":
                ctrl = new Ctrl_ShapeCurve(hwpx, "ruc$", child, version);
                list.add(ctrl);
                break;
            case "hp:connectLine":
                ctrl = new Ctrl_ShapeLine(hwpx, "loc$", child, version);
                list.add(ctrl);
                break;
            case "hp:pic":
                ctrl = new Ctrl_ShapePic(hwpx, "cip$", child, version);
                list.add(ctrl);
                break;
            case "hp:ole":
                ctrl = new Ctrl_ShapeOle(hwpx, "elo$", child, version);
                list.add(ctrl);
                break;
            case "#text":
                break;
